...
echo 'Beantragte Auszahlungen:<br><br>';
$getop=mysql_query("SELECT * FROM ".$dbx."_auszahlung WHERE status='' ORDER BY datum DESC");
if(mysql_num_rows($getop)==0) echo '<i>keine Auszahlungen beantragt.</i>';
else {
echo '<form action=index.php method=post><input type=hidden name=d value=auszahlungen><input type=hidden name=o value=k><select name=auszahlung>';
...
echo 'Beantragte Verifizierungen:<br><br>';
$getop=mysql_query("SELECT * FROM ".$dbx."_user WHERE verifizierung='wt' ORDER BY datum DESC");
if(mysql_num_rows($getop)==0) echo '<i>keine Verifizierungen beantragt.</i>';
else {
echo '<form action=index.php method=post><input type=hidden name=d value=verifizierung><input type=hidden name=o value=k><select name=verifizierung>';
while($op=mysql_fetch_array($getop)) {
