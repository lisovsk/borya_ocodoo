...
if ($xfbuser) {
    ...
    $loginUrl = $facebook->getLoginUrl($params = array('scope' => 'email,user_location'));
...
$vorname = mysqli_real_escape_string($db,utf8_decode($userdata['first_name']));
$nachname = mysqli_real_escape_string($db,utf8_decode($userdata['last_name']));
$email = $userdata['email'];
$geschlecht = $userdata['gender'];
$geburtsdatum = $userdata['birthday'];
$ort = mysqli_real_escape_string($db,utf8_decode($userdata['location']['name']));

$getuudd=mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE email='".$email."'");
$uudd=mysqli_fetch_assoc($getuudd);
$getffdd=mysqli_query($db,"SELECT * FROM ".$dbx."_user_facebook WHERE facebook_id='".$facebook_id."'");
$ffdd=mysqli_fetch_assoc($getffdd);
...
$getuna=mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE user='".$username."'");
$una=mysqli_fetch_assoc($getuna);
if($una[id]=="") $a_user=$username;
else {
$a=1; while($a < 9999) { $a++;
$getuna=mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE user='".$username.$a."'");
$una=mysqli_fetch_assoc($getuna);
...
mysqli_query($db,"INSERT INTO ".$dbx."_user (user,pass,vorname,nachname,land,plz,ort,email,datum,online,geschlecht,ip) VALUES ('".addslashes($a_user)."','".$a_pass."','".addslashes($vorname)."','".addslashes($nachname)."','".addslashes($a_land)."','".$a_plz."','".addslashes($a_ort)."','".addslashes($email)."','".time()."','".time()."','".addslashes($a_geschlecht)."','".$usrip."')");

$einloggen = mysqli_insert_id($db);

mysqli_query($db,"INSERT INTO ".$dbx."_user_facebook (facebook_id,userid) VALUES ('".$facebook_id."','".$einloggen."')");
...
$getuudd=mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE id='".$einloggen."'");
$uudd=mysqli_fetch_assoc($getuudd);
