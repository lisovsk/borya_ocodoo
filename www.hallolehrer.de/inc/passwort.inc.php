...
$getpwdata = mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE email='".$pw_email."'");
$pwdata = mysqli_fetch_array($getpwdata);
if($pwdata[id]=="") echo genMsg('images/ico_error.png','E-Mail-Adresse ungültig.');
else {
srand(date("s")); while($i<8) { $a_pass.=chr((rand()%26)+97); $i++; }

mysqli_query($db,"UPDATE ".$dbx."_user SET pass='".$a_pass."' WHERE id='".$pwdata[id]."'");
