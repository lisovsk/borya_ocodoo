...
if($s=="posteingang") $getmsgs = mysqli_query($db,"SELECT * FROM ".$dbx."_nachrichten WHERE empf='".$usrd[id]."' AND delempf NOT LIKE 'ok' ORDER BY datum DESC");
elseif($s=="postausgang") $getmsgs = mysqli_query($db,"SELECT * FROM ".$dbx."_nachrichten WHERE abs='".$usrd[id]."' AND delabs NOT LIKE 'ok'ORDER BY datum DESC");


if(mysqli_num_rows($getmsgs)==0) echo '<br><div class="alert alert-danger"><b>Ups!</b> Keine Nachrichten gefunden.</div>';
...
while($msgs=mysqli_fetch_array($getmsgs)) {

if($s=="posteingang") $getabsuser = mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE id='".$msgs[abs]."'");
elseif($s=="postausgang") $getabsuser = mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE id='".$msgs[empf]."'");
$absuser = mysqli_fetch_array($getabsuser);
...
mysqli_query($db,"UPDATE ".$dbx."_nachrichten SET delempf='ok' WHERE id='".$u."' AND empf='".$usrd[id]."'");
mysqli_query($db,"UPDATE ".$dbx."_nachrichten SET delabs='ok' WHERE id='".$u."' AND abs='".$usrd[id]."'");
...
$getmsg = mysqli_query($db,"SELECT * FROM ".$dbx."_nachrichten WHERE id='".$u."' AND (empf='".$usrd[id]."' OR abs='".$usrd[id]."')");
$msg=mysqli_fetch_array($getmsg);

if($msg[id]=="") echo '<div class="alert alert-danger"><b>Ups!</b> Unbekannter Fehler.</div>';
else {

$getabsuser = mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE id='".$msg[abs]."'");
$absuser = mysqli_fetch_array($getabsuser);

mysqli_query($db,"UPDATE ".$dbx."_nachrichten SET status='ok' WHERE id='".$msg[id]."'");
...
$getempfuser = mysqli_query($db,"SELECT * FROM ".$dbx."_user WHERE id='".$send."'");
$empfuser = mysqli_fetch_array($getempfuser);
...
mysqli_query($db,"INSERT INTO ".$dbx."_nachrichten (empf,abs,betr,cont,datum) VALUES ('".$empfuser[id]."','".$usrd[id]."','".$a_betreff."','".$a_nachricht."','".time()."')");

$ph1=array('%vorname%','%nachname%','%readlink%','%titel%','%weburl%');
$ph2=array($empfuser[vorname],$empfuser[nachname],'http://'.$coredata[url].'?d=nachrichten&s=lesen&u='.mysqli_insert_id($db),$coredata[titel],$coredata[url]);
...
echo '<form action=index.php method=post class=form-horizontal accept-charset="UTF-8"><input type=hidden name=d value=nachrichten><input type=hidden name=s value=senden><input type=hidden name=u value="'.$empfuser[id].'"><input type=hidden name=o value=k>
