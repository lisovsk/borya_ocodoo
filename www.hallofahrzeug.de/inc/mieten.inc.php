...
elseif($checkdd[id]!="") echo '<div class="alert alert-danger"><b>Ups!</b> Das Fahrzeug ist im gewählten Zeitraum leider nicht verfügbar.</div>'.$neueDatumwahl;
...
elseif($usrd[id]==$fahrzeug[user]) echo '<div class="alert alert-danger"><b>Ups!</b> Sie können nicht Ihr eigenes Fahrzeug buchen.</div>';
else {
...
echo '<div class="alert alert-danger"><b>Ups!</b> Sie haben nicht genügend Guthaben.</div><div>Wir versuchen das Mieten und Vermieten über '.$coredata[titel].' möglichst unkompliziert und sicher abzuwickeln. Aus diesem Grund ist die Bezahlung direkt bei der Buchungsanfrage fällig. Falls die Buchung nicht zustande kommt, kann der Betrag wieder voll ausgezahlt oder für die Buchung eines anderen Fahrzeuges verwendet werden. Weitere Infos zum Ablauf: <a href="'.genURL('faq').'">Häufige Fragen</a><br><br>Die angefragten '.$mietdauer.' Tage dieses Fahrzeuges kosten '.$mietpreis_total.',00 '; if($coredata['waehrung']=="EUR") echo '&euro;'; else echo $coredata['waehrung']; echo '.<br><br>Bitte laden Sie <a href="'.genURL('konto').'">hier Ihr Konto</a> auf und starten Sie die Buchungsanfrage erneut.</div>';
}
