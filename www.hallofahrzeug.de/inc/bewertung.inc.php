...
$getfahrzeug = mysql_query("SELECT * FROM ".$dbx."_fahrzeug WHERE id='".$fetchBewertung[fahrzeug]."'");
$fahrzeug = mysql_fetch_array($getfahrzeug);

$getbuser=mysql_query("SELECT * FROM ".$dbx."_user WHERE id='".$fetchBewertung[user]."'");
$buser = mysql_fetch_array($getbuser);

if($buser[id]!="" && $fahrzeug[status]=="ok") {
...
<tr><td>Fahrzeug:</td><td><a href="'.genURL('fahrzeug',$fahrzeug[id]).'">'.$fahrzeug[titel].'</a></td></tr>
