...
$getunterkunft = mysql_query("SELECT * FROM ".$dbx."_unterkunft WHERE id='".$oldma[unterkunft]."'");
...
$getgg = mysql_query("SELECT * FROM ".$dbx."_unterkunft WHERE status='ok' ORDER BY datum DESC");

while($gg=mysql_fetch_array($getgg)) {
$xmlsitemap.='
<url>
        <loc>'.genURL('unterkunft',$gg[id],urlseotext($gg[titel])).'</loc>
        <priority>0.9</priority>
        <changefreq>weekly</changefreq>
        <lastmod>'.date("Y-m-d").'</lastmod>
</url>';
