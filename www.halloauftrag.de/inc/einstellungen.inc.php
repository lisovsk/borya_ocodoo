...
$a_firma=htmlspecialchars(stripslashes(ucfirst($_POST[a_firma])));
...
mysqli_query($db,"UPDATE ".$dbx."_user SET email='".addslashes($a_email)."', newsletter='".addslashes($a_newsletter)."', ip='".$usrip."' WHERE id='".$usrd[id]."'");
if($a_pass1!="" && $a_pass1==$a_pass2) mysqli_query($db,"UPDATE ".$dbx."_user SET pass='".addslashes($a_pass1)."', ip='".$usrip."' WHERE id='".$usrd[id]."'");
...
mysqli_query($db,"UPDATE ".$dbx."_user SET firma='".addslashes($a_firma)."', vorname='".addslashes($a_vorname)."', nachname='".addslashes($a_nachname)."', land='".addslashes($a_land)."', plz='".$a_plz."', ort='".addslashes($a_ort)."', geschlecht='".$a_geschlecht."', ip='".$usrip."' WHERE id='".$usrd[id]."'");
...
echo "<form action=index.php method=post enctype=multipart/form-data class=form-horizontal accept-charset=\"UTF-8\"><input type=hidden name=d value=einstellungen><input type=hidden name=o value=k><table border=0 cellspacing=0 cellpadding=5>
...
<div class=form-group>
<label for=a_ort class=\"col-lg-4 col-sm-4 control-label\">&nbsp;</label><div class=\"col-lg-8 col-sm-8\">
<img src=images/ico_user_w.png width=16 height=16 align=absmiddle> <input type=radio name=a_geschlecht value=w id=w"; if($usrd[geschlecht]=="w") echo " checked"; echo "> <label for=w>Frau</label><br>
<img src=images/ico_user_m.png width=16 height=16 align=absmiddle> <input type=radio name=a_geschlecht value=m id=m"; if($usrd[geschlecht]=="m") echo " checked"; echo "> <label for=m>Mann</label><br>
<img src=images/ico_user_c.png width=16 height=16 align=absmiddle> <input type=radio name=a_geschlecht value=c id=c"; if($usrd[geschlecht]=="c") echo " checked"; echo "> <label for=m>Firma</label>
</div></div>

<div class=form-group>
<label for=a_firma class=\"col-lg-4 col-sm-4 control-label\">Firma</label><div class=\"col-lg-8 col-sm-8\">
<input type=text id=a_firma name=a_firma style=width:100%;max-width:350px; value=\"".$usrd[firma]."\" class=form-control>
</div></div>
