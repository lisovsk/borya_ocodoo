...
$getbb=mysqli_query($db,"SELECT user,email FROM ".$dbx."_user WHERE user='".$a_user."' OR email='".$a_email."'");
$getb=mysqli_fetch_assoc($getbb);
...
mysqli_query($db,"INSERT INTO ".$dbx."_user (user,pass,firma,vorname,nachname,land,plz,ort,email,datum,online,geschlecht,ip) VALUES ('".addslashes($a_user)."','".$a_pass."','".addslashes($a_firma)."','".addslashes($a_vorname)."','".addslashes($a_nachname)."','".addslashes($a_land)."','".$a_plz."','".addslashes($a_ort)."','".addslashes($a_email)."','".time()."','".time()."','".$a_geschlecht."','".$usrip."')");

$nuid=mysqli_insert_id($db);
...
<form action=index.php?session_code=".$session_code." method=post class=form-horizontal accept-charset=\"UTF-8\"><input type=hidden name=d value=anmelden><input type=hidden name=o value=k>";
...
<img src=images/ico_user_w.png width=16 height=16 valign=absmiddle>&nbsp;<input type=radio name=a_geschlecht checked value=w id=w>&nbsp;<label for=w>Frau</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=images/ico_user_m.png width=16 height=16 valign=absmiddle>&nbsp;<input type=radio name=a_geschlecht value=m id=m>&nbsp;<label for=m>Herr</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src=images/ico_user_c.png width=16 height=16 valign=absmiddle>&nbsp;<input type=radio name=a_geschlecht value=c id=c>&nbsp;<label for=c>Firma</label>
</div></div>

<div class=form-group>
<label for=a_firma class=\"col-lg-5 col-sm-4 control-label\">Firma</label><div class=\"col-lg-7 col-sm-8\">
<input type=text id=a_firma name=a_firma style=width:100%;max-width:350px; value=\"".$a_firma."\" class=form-control>
</div></div>
