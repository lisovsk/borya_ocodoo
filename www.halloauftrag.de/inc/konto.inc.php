...
if($s=="danke") {
echo genMsg('images/ico_ok.png','Danke für die Zahlung.').'<br><br>Es kann einige Minuten dauern, bis der Betrag oder die Flatrate auf Ihrem Konto gutgeschrieben wurde.';
}
...
elseif($buchung[buchung]=="angebot") echo 'Angebot für <a href="'.genURL('auftrag',$buchung[auftrag]).'">Auftrag</a> abgegeben';
elseif($buchung[buchung]=="flatrate") echo 'Flatrate gebucht';
...
<div style="padding:5px;background-color:#eee;margin-bottom:10px;" class=smallroundcorners><b>Flatrate</b></div>
<div style="padding:5px;margin-bottom:20px;">';


if($usrd[flatrate]>time()) echo '<div style="padding:5px;margin-bottom:20px;">Flatrate gebucht bis:<br><b>'.date("j.n.Y, H.i",$usrd[flatrate]).' Uhr</b></div>';
else {
echo '<div style="padding:5px;margin-bottom:20px;">
<form action=payredirect.php method=post><input type=hidden name=userkonto value='.$usrd[id].'><input type=hidden name=psp value="">Flatrate buchen: <select name=flatrate style=width:150px>

<option value=1>30 Tage für '.$coredata['flatrate1'].' '; if($coredata['waehrung']=="EUR") echo '&euro;'; else echo $coredata['waehrung']; echo '</option>
<option value=2>90 Tage für '.$coredata['flatrate2'].' '; if($coredata['waehrung']=="EUR") echo '&euro;'; else echo $coredata['waehrung']; echo '</option>
<option value=3>1 Jahr für '.$coredata['flatrate3'].' '; if($coredata['waehrung']=="EUR") echo '&euro;'; else echo $coredata['waehrung']; echo '</option>

</select><br><br>Zahlung ausführen mit:<br>';
if($coredata['paypal']=="ja") echo '<input type=image src=images/logo_paypal.png width=83 height=28 alt="PayPal" name=psp value=pp style="padding:2px;border:1px solid #ccc;margin-right:5px;" onclick="this.form.psp.value=\'pp\';this.form.submit()" class=smallroundcorners>';
if($coredata['sofortueberweisung']=="ja") echo '<input type=image src=images/logo_sofortueberweisung.png width=83 height=28 alt="Sofortüberweisung" name=psp value=ue style="padding:2px;border:1px solid #ccc;" onclick="this.form.psp.value=\'su\';this.form.submit()" class=smallroundcorners>';
