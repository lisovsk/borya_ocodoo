...
if($s=="danke") {
echo genMsg('images/ico_ok.png','Danke für die Einzahlung.').'<br><br>Es kann einige Minuten dauern, bis der Betrag auf Ihrem Konto gutgeschrieben wurde.';
}
...
mysql_query("UPDATE ".$dbx."_user SET auszahlungsart='".addslashes($aart)."', paypal='".addslashes($paypal)."', bank_name='".addslashes($bank_name)."', bank_konto='".addslashes($bank_konto)."', bank_blz='".addslashes($bank_blz)."', bank_inhaber='".addslashes($bank_inhaber)."', guthaben='0' WHERE id='".$usrd[id]."'");

mysql_query("INSERT INTO ".$dbx."_auszahlung (user,betrag,datum) VALUES ('".$usrd[id]."','".$usrd[guthaben]."','".time()."')");

mysql_query("INSERT INTO ".$dbx."_konto (user,buchung,betrag,datum,status) VALUES ('".$usrd[id]."','auszahlung','-".$usrd[guthaben]."','".time()."','ok')");
...
$getbuchungen=mysql_query("SELECT * FROM ".$dbx."_konto WHERE user='".$usrd[id]."' AND status='ok' ORDER BY datum DESC LIMIT 0,20");
if(mysql_num_rows($getbuchungen)==0) echo '<i>Bisher keine Kontobewegungen.</i>';
else {
...
elseif($buchung[buchung]=="miete") echo '<a href="'.genURL('gegenstand',$buchung[gegenstand]).'">Gegenstand</a> gemietet';
elseif($buchung[buchung]=="vermietung") echo '<a href="'.genURL('gegenstand',$buchung[gegenstand]).'">Gegenstand</a> vermietet';
elseif($buchung[buchung]=="rueckerstattung") echo 'Rückerstattung: Vermietung des <a href="'.genURL('gegenstand',$buchung[gegenstand]).'">Gegenstandes</a> abgebrochen';
...
<div style="padding:5px;background-color:#eee;margin-bottom:10px;" class=smallroundcorners><b>Konto aufladen</b></div>
<div style="padding:5px;margin-bottom:20px;">
